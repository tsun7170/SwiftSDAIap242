/* file: text_string_representation_item.swift 	 generated: Fri Sep 26 17:24:11 2025 */
...
*/
  public enum sTEXT_STRING_REPRESENTATION_ITEM : SDAIValue, AP242_MANAGED_MODEL_BASED_3D_ENGINEERING_MIM_LF__sTEXT_STRING_REPRESENTATION_ITEM__type {
    ...
    /// SELECT case ``eANNOTATION_TEXT`` (ENTITY) in ``sTEXT_STRING_REPRESENTATION_ITEM``
    case _ANNOTATION_TEXT(eANNOTATION_TEXT)	// (ENTITY)

    /// SELECT case ``eANNOTATION_TEXT_CHARACTER`` (ENTITY) in ``sTEXT_STRING_REPRESENTATION_ITEM``
    case _ANNOTATION_TEXT_CHARACTER(eANNOTATION_TEXT_CHARACTER)	// (ENTITY)

    /// SELECT case ``sAXIS2_PLACEMENT`` (SELECT) in ``sTEXT_STRING_REPRESENTATION_ITEM``
    case _AXIS2_PLACEMENT(sAXIS2_PLACEMENT)	// (SELECT)

    /// SELECT case ``eCOMPOSITE_TEXT`` (ENTITY) in ``sTEXT_STRING_REPRESENTATION_ITEM``
    case _COMPOSITE_TEXT(eCOMPOSITE_TEXT)	// (ENTITY)

    /// SELECT case ``eDEFINED_CHARACTER_GLYPH`` (ENTITY) in ``sTEXT_STRING_REPRESENTATION_ITEM``
    case _DEFINED_CHARACTER_GLYPH(eDEFINED_CHARACTER_GLYPH)	// (ENTITY)

    /// SELECT case ``eTEXT_LITERAL`` (ENTITY) in ``sTEXT_STRING_REPRESENTATION_ITEM``
    ...
    /// attribute of SELECT type ``sTEXT_STRING_REPRESENTATION_ITEM``

/* file: annotation_text_occurrence_item.swift 	 generated: Fri Sep 26 17:24:11 2025 */
...
*/
  public enum sANNOTATION_TEXT_OCCURRENCE_ITEM : SDAIValue, AP242_MANAGED_MODEL_BASED_3D_ENGINEERING_MIM_LF__sANNOTATION_TEXT_OCCURRENCE_ITEM__type {
    ...
    /// SELECT case ``eANNOTATION_TEXT`` (ENTITY) in ``sANNOTATION_TEXT_OCCURRENCE_ITEM``
    case _ANNOTATION_TEXT(eANNOTATION_TEXT)	// (ENTITY)

    /// SELECT case ``eANNOTATION_TEXT_CHARACTER`` (ENTITY) in ``sANNOTATION_TEXT_OCCURRENCE_ITEM``
    case _ANNOTATION_TEXT_CHARACTER(eANNOTATION_TEXT_CHARACTER)	// (ENTITY)

    /// SELECT case ``eCOMPOSITE_TEXT`` (ENTITY) in ``sANNOTATION_TEXT_OCCURRENCE_ITEM``
    case _COMPOSITE_TEXT(eCOMPOSITE_TEXT)	// (ENTITY)

    /// SELECT case ``eDEFINED_CHARACTER_GLYPH`` (ENTITY) in ``sANNOTATION_TEXT_OCCURRENCE_ITEM``
    case _DEFINED_CHARACTER_GLYPH(eDEFINED_CHARACTER_GLYPH)	// (ENTITY)

    /// SELECT case ``eTEXT_LITERAL`` (ENTITY) in ``sANNOTATION_TEXT_OCCURRENCE_ITEM``
    ...
    /// attribute of SELECT type ``sANNOTATION_TEXT_OCCURRENCE_ITEM``

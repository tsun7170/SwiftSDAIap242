/* file: shape_representation_with_parameters_items.swift 	 generated: Fri Sep 26 17:24:11 2025 */
...
*/
  public enum sSHAPE_REPRESENTATION_WITH_PARAMETERS_ITEMS : SDAIValue, AP242_MANAGED_MODEL_BASED_3D_ENGINEERING_MIM_LF__sSHAPE_REPRESENTATION_WITH_PARAMETERS_ITEMS__type {
    ...
    /// SELECT case ``eDESCRIPTIVE_REPRESENTATION_ITEM`` (ENTITY) in ``sSHAPE_REPRESENTATION_WITH_PARAMETERS_ITEMS``
    case _DESCRIPTIVE_REPRESENTATION_ITEM(eDESCRIPTIVE_REPRESENTATION_ITEM)	// (ENTITY)

    /// SELECT case ``eDIRECTION`` (ENTITY) in ``sSHAPE_REPRESENTATION_WITH_PARAMETERS_ITEMS``
    case _DIRECTION(eDIRECTION)	// (ENTITY)

    /// SELECT case ``eMEASURE_REPRESENTATION_ITEM`` (ENTITY) in ``sSHAPE_REPRESENTATION_WITH_PARAMETERS_ITEMS``
    case _MEASURE_REPRESENTATION_ITEM(eMEASURE_REPRESENTATION_ITEM)	// (ENTITY)

    /// SELECT case ``ePLACEMENT`` (ENTITY) in ``sSHAPE_REPRESENTATION_WITH_PARAMETERS_ITEMS``
    ...
    /// attribute of SELECT type ``sSHAPE_REPRESENTATION_WITH_PARAMETERS_ITEMS``

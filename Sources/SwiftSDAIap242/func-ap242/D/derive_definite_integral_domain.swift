/* file: derive_definite_integral_domain.swift 	 generated: Fri Sep 26 17:24:11 2025 */
...
*/
  ...
  func DERIVE_DEFINITE_INTEGRAL_DOMAIN(
    ...
    func PROCESS_PRODUCT_SPACE(SPC: sPRODUCT_SPACE? , IDX: SDAI.INTEGER? , 
      ...
      if SDAI.IS_TRUE( _TEMP3 ) {
        ...
        if SDAI.IS_TRUE( _TEMP9 ) {
          ...
          let _TEMP10 = SDAI.LIST<eMATHS_SPACE>(SDAI.EMPTY_AGGREGATE)
    ...
    var TUPLED: SDAI.BOOLEAN?  = BOOL(SDAI.LOGICAL(( SPACE_DIMENSION(IDOMN) .==. SDAI.FORCE_OPTIONAL(SDAI.INTEGER(
                                 1)) ) && SDAI.aggregate(TYPES, contains: SDAI.FORCE_OPTIONAL(SCHEMA_PREFIX)
                                 + SDAI.FORCE_OPTIONAL(SDAI.STRING("TUPLE_SPACE"))))); SDAI.TOUCH(var: &TUPLED)
    ...
    let _TEMP9 = SDAI.aggregate(TYPES, contains: _TEMP8)
    if SDAI.IS_TRUE( _TEMP9 ) {

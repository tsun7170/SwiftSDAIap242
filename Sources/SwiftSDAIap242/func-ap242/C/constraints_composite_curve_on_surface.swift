/* file: constraints_composite_curve_on_surface.swift 	 generated: Tue Jan 20 19:20:51 2026 */
...
```express
  ...
  END_FUNCTION; -- constraints_composite_curve_on_surface (line:37161 file:ap242ed2_mim_lf_v1.101_TY.exp)
...
*/
  ...
  public static 
  func CONSTRAINTS_COMPOSITE_CURVE_ON_SURFACE( _ C: eCOMPOSITE_CURVE_ON_SURFACE.PRef?  ) 
    -> SDAI.BOOLEAN? {
    ...
    if let incrementControl/*SDAI.INTEGER*/ = SDAI.FROM(/*SDAI.INTEGER*/1, TO:/*SDAI.INTEGER*/N_SEGMENTS) {
      for K in incrementControl {
        ...
        let _TEMP1 /* SDAI.LOGICAL */ = SDAI.TYPEOF( (C?.GROUP_REF(eCOMPOSITE_CURVE.self))?.SEGMENTS?[K]?.PARENT_CURVE, 
          IS: ePCURVE.self )
        let _TEMP2 /* SDAI.LOGICAL */ =  !_TEMP1
        let _TEMP3 /* SDAI.LOGICAL */ = SDAI.TYPEOF( (C?.GROUP_REF(eCOMPOSITE_CURVE.self))?.SEGMENTS?[K]?.PARENT_CURVE, 
          IS: eSURFACE_CURVE.self )
        let _TEMP4 /* SDAI.LOGICAL */ =  !_TEMP3
        let _TEMP5 /* SDAI.LOGICAL */ = _TEMP2 && _TEMP4
        let _TEMP6 /* SDAI.LOGICAL */ = SDAI.TYPEOF( (C?.GROUP_REF(eCOMPOSITE_CURVE.self))?.SEGMENTS?[K]?.PARENT_CURVE, 
...
private let _constraints_composite_curve_on_surface__cache = SDAI.FunctionResultCache(
  label: "CONSTRAINTS_COMPOSITE_CURVE_ON_SURFACE",

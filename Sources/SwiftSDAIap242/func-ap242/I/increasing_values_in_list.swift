/* file: increasing_values_in_list.swift 	 generated: Fri Feb  6 16:23:46 2026 */

/* This file was generated by the EXPRESS to Swift translator "exp2swift", 
  derived from STEPcode (formerly NIST's SCL).
 exp2swift version: v.2.0.0, derived from stepcode v0.8 as of 2019/11/23 
 WARNING: You probably don't want to edit it since your modifications 
  will be lost if exp2swift is used to regenerate it.
 */

 
import SwiftSDAIcore

extension AP242_MANAGED_MODEL_BASED_3D_ENGINEERING_MIM_LF {
//MARK: -FUNCTION definition in EXPRESS

/** FUNCTION definition
- EXPRESS source code:
```express

  FUNCTION increasing_values_in_list(
               values : LIST [2 : ?] OF REAL
           ) : BOOLEAN;

    LOCAL
      result : BOOLEAN := TRUE;
      limit  : INTEGER := SIZEOF( values );
    END_LOCAL;
    REPEAT i := 2 TO limit BY 1;
      IF values[i] <= values[i - 1] THEN
        result := FALSE;
      END_IF;
    END_REPEAT;
    RETURN( result );

  END_FUNCTION; -- increasing_values_in_list (line:42535 file:ap242ed2_mim_lf_v1.101_TY.exp)

```

*/

  public static 
  func INCREASING_VALUES_IN_LIST( _ VALUES: (SDAI.LIST<SDAI.REAL>/*[2:(nil as SDAI.INTEGER?)]*/ )?  ) 
    -> SDAI.BOOLEAN? {

    //CACHE LOOKUP
    let _params = SDAI.ParameterList( VALUES )
    if case .available(let _cached_value) = _increasing_values_in_list__cache.cachedValue(params: _params) {
      return _cached_value as? SDAI.BOOLEAN
    }

    //PARAMETER VALUES
    var VALUES = VALUES; SDAI.TOUCH(var: &VALUES)

    //LOCAL
    var RESULT: SDAI.BOOLEAN?  = SDAI.BOOLEAN(SDAI.TRUE); SDAI.TOUCH(var: &RESULT)
    var LIMIT: SDAI.INTEGER?  = SDAI.SIZEOF( VALUES ); SDAI.TOUCH(var: &LIMIT)
    //END_LOCAL

    //BODY
    if let incrementControl/*SDAI.INTEGER*/ = SDAI.FROM(/*SDAI.INTEGER*/2, TO:/*SDAI.INTEGER*/LIMIT) {
      for I in incrementControl {
        
        let _TEMP1 /* SDAI.REAL? */ = VALUES?[I]
        let _TEMP2 /* SDAI.INTEGER? */ = SDAI.FORCE_OPTIONAL(I) - SDAI.FORCE_OPTIONAL(SDAI.INTEGER(1))
        let _TEMP3 /* SDAI.REAL? */ = VALUES?[_TEMP2]
        let _TEMP4 /* SDAI.LOGICAL */ = _TEMP1 <= _TEMP3
        if SDAI.IS_TRUE( _TEMP4 ) {
          RESULT = SDAI.BOOLEAN(SDAI.FALSE)
        }
      }
    }
    return _increasing_values_in_list__cache.updateCache(params: _params, value: RESULT)
  }

}

//MARK: - function result cache
private let _increasing_values_in_list__cache = SDAI.FunctionResultCache(
  label: "INCREASING_VALUES_IN_LIST",
  controller: AP242_MANAGED_MODEL_BASED_3D_ENGINEERING_MIM_LF.schemaDefinition )


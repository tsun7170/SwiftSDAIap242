/* file: valid_wireframe_edge_curve.swift 	 generated: Sun Jan 18 16:59:32 2026 */
...
```express
  ...
  END_FUNCTION; -- valid_wireframe_edge_curve (line:48572 file:ap242ed2_mim_lf_v1.101_TY.exp)
...
*/
  ...
  public static 
  func VALID_WIREFRAME_EDGE_CURVE( _ CRV: eCURVE.PRef?  ) 
    -> SDAI.BOOLEAN? {
    ...
    if SDAI.IS_TRUE( _TEMP5 ) {
      ...
      let _TEMP6 /* SDAI.LOGICAL */ = SDAI.TYPEOF( CRV, IS: eCURVE_REPLICA.self )
      if SDAI.IS_TRUE( _TEMP6 ) {
        
        let _TEMP7 /* eCURVE_REPLICA.PRef? */ = CRV?.GROUP_REF(eCURVE_REPLICA.self)
        let _TEMP8 /* eCURVE.PRef? */ = _TEMP7?.PARENT_CURVE
        let _TEMP9 /* SDAI.BOOLEAN? */ = VALID_WIREFRAME_EDGE_CURVE( _TEMP8 )
        return _valid_wireframe_edge_curve__cache.updateCache(params: _params, value: _TEMP9)
      }
      else {
        
        let _TEMP10 /* SDAI.LOGICAL */ = SDAI.TYPEOF( CRV, IS: eOFFSET_CURVE_3D.self )
        if SDAI.IS_TRUE( _TEMP10 ) {
          
          let _TEMP11 /* eOFFSET_CURVE_3D.PRef? */ = CRV?.GROUP_REF(eOFFSET_CURVE_3D.self)
          let _TEMP12 /* eCURVE.PRef? */ = _TEMP11?.BASIS_CURVE
          let _TEMP13 /* SDAI.BOOLEAN? */ = VALID_WIREFRAME_EDGE_CURVE( _TEMP12 )
...
private let _valid_wireframe_edge_curve__cache = SDAI.FunctionResultCache(
  label: "VALID_WIREFRAME_EDGE_CURVE",
